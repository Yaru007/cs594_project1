...
You are provided with utility code in Torch and PETSc, and you must stick to the given framework.

You can work in teams of at most three people.
{\bf Only one member of the team needs to submit the work on Blackboard.}
The due date for this project is {\bf 17:00 on Nov 4, 2016}.
...
This project is worth {\bf 30\%} of your final grade.
...
\begin{enumerate}
  ...
  \item A tarball or zip file which includes your source code.
    Your code should be well commented.
    If you changed the format of command line, then include a short readme.txt file that explains how to run your code.
\end{enumerate}
\vspace{-1em}
...
\vspace{-1em}
\begin{itemize}
  \item {\bf Node weight:} Letter-wise discriminant weight vector $\wvec_k \in \RR^{129}$ for each possible letter label $k \in \Ycal$;
...
T1, T2, etc will be used for the Torch experiment.
...
\section{Torch: Implementation Details and Utilities Provided}
\label{sec:parallel_detail}

You do not have to work from scratch.
The following routines have been provided in the folder \verb!code_Torch!,
allowing you to focus on optimization.
In my implementation, only {\bf 40} new lines were added to complete the implementation of CRF for {\bf Question T1} below.


Currently, the code only provides a skeleton of the program, and it cannot be run as is.
No complete implementation of IID models is provided,
because it really take little effort to enhance it into CRF.

\subsection{Data}

The following data files are provided in the \verb!data! folder:

\verb!X_torch.dat!, \qquad where \verb!X! $\in$ \{\verb!train, test, valid!\}.

The \verb!train! and \verb!test! files correspond to those in the PETSc part (but in a new format).
The file \verb!valid! is a small dataset used for debugging with \verb!checkgrad! (see below).

To load the data, see lines 34-40 in \verb!train_CRF.lua!,
where the variable names are self-explanatory.
The length of the \verb!wLen! list is equal to the number of words,
with \verb!wLen[t]! equal the number of letters in the word \verb!t!.
The \verb!data! field is a tricky table, and its usage can be found in the function \verb!updateOutput! of \verb!LinearCRF.lua!.

\vspace{-0.5em}
\begin{itemize}
  \item \verb!data[i]! returns the \verb!i!-th letter in the whole set.
  \item \verb!data[i][1]! $\in \{1,\ldots,26\}$ is the letter's label.
  \item \verb!data[i][2]! encodes the \emph{sparse} features of the letter \verb!i! as follows.
  \item \verb!data[i][2][1][k]! $\in \{1,\ldots,129\}$ is the index of the $k$-th nonzero feature.  $k$ ranges from 1 to 10 if the letter has only 10 nonzero features.
      Note the bias (id: 129) is already added.
  \item \verb!data[i][2][2][k]! $\in \RR$ is the value of that feature.
\end{itemize}
\vspace{-0.5em}


\subsection{Computation modules}

The code provides two new modules: \verb!LinearCRF.lua! and \verb!ClassCRFCriterion.lua!.
You may consider the whole model as chaining two ``layers":

\vspace{-0.5em}
\begin{itemize}
  \item Layer 1: \verb!LinearCRF.lua!.  Its input is $X^t$, and its output is a \emph{table} with two components (right, the output can be any data format, not necessarily a tensor or vector):
      \begin{align}
        \text{outNode} &= X^t \cdot (\wvec_1, \ldots, \wvec_{26}) = \{ \inner{\xvec^t_s}{\wvec_k}: s \in [1, m], k \in [1,26]\}\in \RR^{m \times 26} \\
        \text{wEdge} &= T \in \RR^{26 \times 26} \quad \text{(yes, independent of the input } X^t).
      \end{align}
  \item Layer 2: \verb!ClassCRFCriterion.lua!. Its input is the above table \{\verb!outNode!, \verb!wEdge!\}, and its output is $\log p(\yvec^t | X^t)$ as shown in \eqref{eq:log_prob}.
\end{itemize}
\vspace{-0.5em}

Note all the learning parameters $\{\wvec_1, \ldots, \wvec_{26}, T\}$ are encapsulated in \verb!LinearCRF.lua!,
while no parameter is contained in \verb!ClassCRFCriterion.lua!.
To utilize Torch's engine of gradient computation,
we only need to instantiate the following routines:

\vspace{-0.5em}
\begin{enumerate}
  \item \verb!LinearCRF:updateOutput!: Given $X^t$, compute the output \{outNode, wEdge\} for the layer of \verb!LinearCRF!.  This has been implemented in the provided package.
      Sparsity in the data $X^t$ has been exploited.
  %
  \item \verb!ClassCRFCriterion:updateOutput!: Given \{outNode, wEdge\}, compute $\log p(\yvec^t | X^t)$---the output of the \verb!ClassCRFCriterion! layer.
      This requires marginal inference on a linear chain, which has been implemented in the provided package.
  %
  \item \verb!ClassCRFCriterion:updateGradInput! (first step of gradient back-propagation): compute the gradient of $\log p(\yvec^t | X^t)$ with respect to \{outNode, wEdge\}.
      These turn out to be exactly the $C^t$ matrix in \eqref{eq:def_Ct}, along with the gradient of $T$ formula in \eqref{eq:grad_edge_weight}.
  %
  \item \verb!LinearCRF:accGradParameters! (second step of gradient back-propagation): compute the gradient of $\log p(\yvec^t | X^t)$ with respect to $\{\wvec_1, \ldots, \wvec_{26}, T\}$ based on the gradient of $\log p(\yvec^t | X^t)$ with respect to \{outNode, wEdge\}---the result of \verb!ClassCRFCriterion:updateGradInput!.
      Indeed, the only computation is the multiplication $G^t = (X^t)^\top C^t$ in \eqref{eq:def_Gt}.
      The gradient in $T$ is simply copied from the wEdge part of the output of \verb!ClassCRFCriterion:updateGradInput!.
\end{enumerate}
\vspace{-0.5em}


You need to implement the last two routines for gradient computation.
In general, the \verb!LinearCRF! should also implement the gradient in its input $X^t$
(to facilitate further back-propagation to its preceding layers).
But since this is not needed for our training, we just put a dummy zero there.

The \verb!feval! function in \verb!train_CRF.lua! demonstrates how to put together these two modules in order to compute the objective value and gradient,
which are used by the LBFGS solver for training.
The MAP inference is implemented in \verb!find_MAP! function,
which is in turn used by the \verb!Monitor! function to compute the training (and test) error at each iteration.


\subsection{Other miscellaneous}

The off-the-shelf \verb!optim.lbfgs! does not print intermediate results.
I hacked it and now it can take a monitor to print training/test errors at each iteration.
See the line \verb!optimState.monitor = monitor! in \verb!train_CRF.lua!.
On cs594.cs.uic.edu, I have recompiled Torch to deploy this hack and you can use it directly.
If you use your own machine, please replace \verb!torch/pkg/optim/lbfgs.lua! with the one provided in the project package.
Then go to your torch directory and run \verb!./install.sh! to recompile Torch.

The format of the printout should be the same as PETSc.
See the bottom of \verb!train_CRF.lua!.
Of course, the numbers change and now the gap column looks less meaningful.
The implementation of LBFGS in PETSc is highly optimized,
and thus it is in general much faster than Torch.
The sample output of my implementation is enclosed as \verb!sample_out_Torch.txt! \emph{just for your reference}.


\section{Torch: Tasks for Experiment}

First complete the implementation of CRF in Torch based on the given framework.
{\bf Your implementation should be filled into \verb!ClassCRFCriterion.lua!}
and \verb!LinearCRF.lua!,
especially \verb!ClassCRFCriterion:updateGradInput! and \verb!LinearCRF:accGradParameters!.
New functions and variables can be added there.
Besides, in \verb!train_CRF.lua!, also add the code of computing the test errors in the function \verb!Monitor!.

{\bf Important: you should always verify that your routine for computing the gradient is correct.}
This is commonly done by comparing your result with the one computed by finite-difference.%
\footnote{See
\url{https://en.wikipedia.org/wiki/Finite\_difference}.
}
The \verb!optim.checkgrad! function does exactly this job.
I also provided three files in the package to help with this check:
\verb!debug_CRFCriterion.lua!,
\verb!debug_LinearCRF.lua!,
and \verb!debug_CRF.lua!.
They are used for testing the gradient of
\verb!ClassCRFCriterion!,
\verb!LinearCRF!,
and their composition (the complete CRF) respectively.
Since finite difference is usually expensive in computation,
this checking can be done on small datasets, \eg\ \verb!debug_torch.dat!.
